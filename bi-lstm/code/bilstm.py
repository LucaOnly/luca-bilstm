...
d['x'] = d['data'].apply(lambda x: np.array(list(chars[x]).extend([0]*(maxlen-len(x))))
...
batch_size = 1024
history = model.fit(np.array(list(d['x'])), np.array(list(d['y'])).values.reshape((-1,maxlen,5)), batch_size=batch_size, nb_epoch=50)
